...
function getInfo(inventory, id) {
    let info = inventory
        .filter( function(inventory) {
            return inventory.id == id;
        });

console.log(`Car ${id} is a ${info[0].car_year} ${info[0].car_make} ${info[0].car_model}` );   
}

getInfo(inventory, 33);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = 0;
lastCar = inventory.splice(-1, 1);
...
console.log(carModels.map(function(models){ return models.toUpperCase()}).sort())
...
function fancyOnly (make) {
    return make.car_make == "Audi" ||
    make.car_make == "BMW";
}

BMWAndAudi = BMWAndAudi.filter(fancyOnly);
